        ...
        <div className="h-75 max-h-[737px] bg-white absolute right-0 bottom-0 left-0 z-[501]">
            ...
            <div className="overflow-y-hidden h-full bg-white overflow-x-hidden ">
                ...
                <div className="mb-0 bg-white border-t border-b border-[#cbcbcb] fixed bottom-0 h-16 m-0 p-2.5 w-full">
                  { activeFilters[0] === false &&  <button onClick={handleClearFilter} className="border-[0.5px] border-[#cbcbcb] text-[#007185] float-left max-w-1/2 rounded-lg inline-block h-10.5 overflow-hidden p-2.5 mb-[13px] text-[15px] leading-[1.35]">Clear Filters</button>}
                    <button onClick={handleFilter} className="rounded-lg mb-0 inline-block h-10.5 overflow-hidden p-2.5 bg-[#007185] text-white max-w-[45%] absolute right-2.5 " >
                        
                        {activeFilters[0] === false ? <span className="text-[15px] leading-[1.35] relative bottom-0.5">Apply Filters</span>:<span className="text-[15px] leading-[1.35] relative bottom-0.5">Show Products</span>}
                    </button>
                </div>
            </div>
        </div>
