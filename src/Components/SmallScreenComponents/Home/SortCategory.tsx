  ...
  return (
    <div className="sticky top-12 bg-[transparent] shadow-[0_1px_2px_0_hsla(0,0%,7%,.15)] -ml-2 -mr-2 z-[208] !mb-2  ">
        <div className="mb-0 bg-white flex ml-2 mr-0 overflow-x-auto pb-2 pt-2 scroll-width-none">
            { (selecedCategory.length > 0 || Object.values(selectedPrice).length > 0 )  &&  <div className={`mr-2 border rounded-[7px] p-[4px_8px] text-center whitespace-nowrap w-auto  bg-white  border-[#e8e8e8]`}>
                <button onClick={handleClearFilter} className={`flex gap-1 -ml-[3px] -mr-[3px] !text-[15px] !leading-[1.35] text-[#111]`} >
                    Clear All 
                </button>
            </div>}
            <div className={`mr-2 border rounded-[7px] p-[4px_8px] text-center whitespace-nowrap w-auto ${selecedCategory.length > 0 ? 'bg-[#e7fbff]  border-[#bddee3] ':' bg-white border border-[#e8e8e8] '}`}>
                <button className={`flex gap-1 -ml-[3px] -mr-[3px] !text-[15px] !leading-[1.35] ${selecedCategory.length > 0 ?'text-[#007185]':'text-[#111]'}`} onClick={handleCateShow}>
                    Categories {selecedCategory.length > 0 ? `(${selecedCategory.length})`:''} <span className="flex items-center"><MdKeyboardArrowDown className="h-[15px] w-[15px]"/></span>
                </button>
            </div>
                {
