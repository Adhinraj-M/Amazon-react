...
function FilterSection({handleFilter,filterCategory}:filterSectionType) {
  ...
  const handleActiveFilter=(index:number,item:string,filterTitle:string)=>{
      ...
      <div className="bg-[rgba(0,0,0,.2)] opacity-100 h-full w-full absolute"></div>
              ...
              filterCategory && filterCategory.map((item,index)=>{
                return(
                  <div key={index} className={`${activeIndex === index ? 'border-b-[#e3e6e6] items-center border-b z-[1] bg-white':'bg-[#fff9f6] border-b-[#fee4d9] items-center border-b z-[1]'} `} onClick={()=>handleCategory(index)}>
                    <div className={`p-[8px_12px] flex  min-h-13 mb-0 relative  items-center text-ellipsis w-23 leading-[1.4] text-[13px] ${activeIndex === index ? '  text-[#eb1745] font-fontBold before:bg-[#eb1745] before:w-1 before:h-full before:content-[""] before:absolute before:left-0 before:right-0':
            ...
            <div className="p-[0_12px]">
              <h2 className="text-[15px] !leading-[1.35] font-fontBold !pb-4">{filterCategory[activeIndex].filter_Type}</h2>
                ...
                <div className="mb-1 flex flex-wrap">
                  {
                    filterBtn.filter_buttons.length > 0 ? (filterBtn.filter_buttons.map((item,index)=>{
                      return(
                        ...
                        <div key={index} className="">
                          {
                            ...
                            <div className="mb-[17px] mr-6 p-[12px_9px] bg-[#f4f4f4] rounded-lgrelative w-[207px]">
                              <div className="flex pb-1 mb-1">
                                <span className="mr-1 p-0 font-fontBold text-[15px] leading-[1.35]">₹{sliderValue.sliderOne}</span>
                                <span className="mr-1 p-0 font-fontBold text-[15px] leading-[1.35]">–</span>
                                <span className="mr-1 p-0 font-fontBold text-[15px] leading-[1.35]">₹{sliderValue.sliderTwo}+</span>
                              </div>
                              ...
                              <div className="flex pb-2 mb-1">
                                <div className="flex-1 p-[10px_0] my-3 relative">
                                  ...
                                  <div className="absolute h-1 bg-[#007185]  translate-y-1/2  rounded-[5px]" style={{left:`${left}%`,width:`${width}%`}}></div>
                                  <input type="range" min={"115"} max={"520"} name="sliderOne" value={sliderValue.sliderOne} onChange={handleSliderValue} className="input-range cursor-pointer absolute pointer-events-none bg-transparent z-[2] text-[#bbbfbf] appearance-none w-full h-1 leading-[1.35]"  />
                                  <input type="range" min={"185"} max={"570"} name="sliderTwo" value={sliderValue.sliderTwo} onChange={handleSliderValue} className="input-range cursor-pointer absolute pointer-events-none bg-transparent z-[3] text-[#bbbfbf] appearance-none w-full h-1 leading-[1.35]"  />
                                </div>
                              </div>
                            </div>
                            )
                          }
                          <button  onClick={()=>handleActiveFilter(index,item,filterBtn.filter_Type)} className={`p-[7px_8px] rounded-lg w-fit mb-2 mr-2 border  text-center inline-flex items-center justify-center text-[13px] leading-[1.4] ${activeFilter[filterBtn.filter_Type] !== index ? '  border-[#e9e8e8] bg-white ':'bg-[#f9d52a]  border-[#f34552] '}`} >
                      ...
                      <button onClick={()=>handleRating()} className={`mr-2 shrink-0 whitespace-nowrap h-8 flex items-center p-[9px] rounded-lg w-auto border  text-center  justify-center${activeFilter['Customer Rating'] !== 0 ? ' border-[#e8e8e8] bg-white ':' bg-[#f9d52a] border-[#f34552]'  }`}>
       ...
       <div className="border-t border-t-[#f34552] fixed w-full bottom-0 bg-white m-0 h-15.5 p-3 z-[600] flex justify-center items-center">
        <div className="flex h-fit w-full justify-between ">
          <button className="flex w-24.5 border p-[10px_12px] border-[#f4d8da]  rounded-lg justify-center items-center bg-white text-[13px] leading-[1.4] shadow-[0px_2px_5px_0px_rgba(213,217,217,0.5)]" onClick={handleClearFilter}>Clear Filters </button>
          <button className="flex w-35  border p-[10px_12px] border-[#ff4d5b]  rounded-lg justify-center items-center bg-[#ff4d5b] text-[13px] text-white leading-[1.4] shadow-[0px_2px_5px_0px_rgba(213,217,217,0.5)]">Show 6,126 results</button>
        </div>
       </div>
    </div>
  )
